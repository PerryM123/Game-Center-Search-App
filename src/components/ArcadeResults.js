    ...
    axios.get('/arcadeData.json')
     .then((results) => {
      ...
      if ( results.data.arcades ) {
        ...
        const arcadeRenderData = arcadeMatchList.map((arcadeItem, arcadeKey) => {
          return (
    ...
    return (
      <div className="contents contents--arcade-results">
        <h2>Arcade Results.js</h2>
        {
          ...
          */
          (this.state.loading) ? <div className="loading_now"><img src={loading_logo} alt="loading-icon" /></div> : <ArcadeRender arcadeData={this.state.arcadeData} />
        }
        <p></p>
      </div>
    );
